...
Array.prototype.shuffle = function () {
  ...
  for (var i = result.length - 1; i >= 0; i--) {
    ...
    var _ref = [result[random], result[i]];
    result[i] = _ref[0];
...
Array.prototype.multiple = function (number) {
  var result = [];

  for (var i = 0; i < number; i++) {
    result = result.concat(this.slice(0));
  }
...
$(function () {
  ...
  $('.inventory_content_list_item').click(function () {
    $(this).toggleClass('inventory_content_list_item-active');
  });

  $('.flip-add_list_item').click(function () {
    $(this).toggleClass('flip-add_list_item-active');
  });
});
...
var Window = function () {
  function Window(options) {
    ...
    var prefix = {
      header: '_header',
      toggle: '_header_toggle',
      hide: '-hide',
      animate: '-animate'
    };

    var defaultOptions = {
      header: options.block + prefix.header,
      toggle: options.block + prefix.toggle,
      hideClass: options.block + prefix.hide,
      animateClass: options.block + prefix.animate,
      togglable: true,
      draggable: true,
      hidden: false
    };
  ...
  _createClass(Window, [{
    ...
    value: function getContentHeight() {
      return $(this.block).height() - $(this.header).height();
    }
  }, {
    key: 'show',
    value: function show() {
      $(this.block).removeClass(this.hideClass.className()).css({
        bottom: 0
      });
    }
  }, {
    key: 'hide',
    value: function hide() {
      $(this.block).addClass(this.hideClass.className()).css({
        bottom: 0 - this.getContentHeight()
      });
    }
  }, {
    key: 'toggleHandler',
    value: function toggleHandler() {
      if ($(this.block).hasClass(this.hideClass.className())) {
        this.show();
      } else {
        this.hide();
      }
    ...
    key: 'dragHandler',
    value: function dragHandler(data) {
      var limitX = $(document).width() - $(this.block).width();
      var positionX = Math.min(limitX, Math.max(0, data.offsetX));
    ...
    key: 'dragBind',
    value: function dragBind() {
      var _this2 = this;

      $(this.block).drag('start', function (event, data) {
        data.height = $(_this2.block).height();
      }).drag(function (event, data) {
        _this2.dragHandler(data);
      }, {
        handle: $(this.header)
      });
    }
    ...
    value: function toggleBind() {
      ...
      $(this.toggle).click(function (event) {
        ...
        _this3.toggleHandler();
      });
    }
  }, {
    key: 'animateEvent',
    value: function animateEvent() {
      $(this.block).addClass(this.animateClass.className());
    }
  }]);
...
var Bar = function () {
  function Bar(options) {
    ...
    var defaultOptions = {
      progress: options.block + prefix.progress,
      delay: 1000
    };
  ...
  _createClass(Bar, [{
    key: 'set',
    value: function set(percent, callback) {
      $(this.progress).css({
        width: percent + '%'
      });
...
var Circle = function () {
  ...
  _createClass(Circle, [{
    ...
    value: function set(percent, callback) {
      $(this.progress).css({
        strokeDashoffset: this.offset * (1 - percent * 0.01)
      });
...
var Roulette = function () {
  function Roulette(options) {
    ...
    var prefix = {
      ...
      show: '-show',
      hide: '-hide',
      winnerName: '_info_user_name',
      winnerLuck: '_info_user_luck',
      winnerImage: '_info_image',
      winnerBank: '_bank_money'
    };

    var defaultOptions = {
      item: options.block + prefix.item,
      image: options.block + prefix.image,
      done: options.parent + prefix.done,
      winnerName: options.parent + prefix.winnerName,
      winnerLuck: options.parent + prefix.winnerLuck,
      winnerImage: options.parent + prefix.winnerImage,
      winnerBank: options.parent + prefix.winnerBank,
      list: options.container + prefix.list,
      hide: options.container + prefix.list + prefix.hide,
      show: options.parent + prefix.show,
      winnerPlace: 17,
      delay: 5000,
      template: '<div class="{itemClass}"><img class="{itemImageClass}" src="{userImage}" data-name="{userName}" data-luck="{userLuck}"></div>'
    };
  ...
  _createClass(Roulette, [{
    key: 'parseItem',
    value: function parseItem(user) {
      var replacePattern = {
        itemClass: this.item.className(),
        itemImageClass: this.image.className(),
      ...
      $.each(replacePattern, function (index, value) {
        item = item.replace('{' + index + '}', replacePattern[index]);
      });
    ...
    key: 'prepareItems',
    value: function prepareItems(users, winner) {
      users = users.multiple(10);
      users = users.shuffle();

      for (var i = 0; i < users.length; i++) {
        users[i] = this.parseItem(users[i]);
      }

      users[this.winnerPlace] = this.parseItem(winner);

      return users;
    }
  }, {
    key: 'getRollDistance',
    value: function getRollDistance() {
      return 0 - $(this.item).outerWidth(true) * (this.winnerPlace - 1.5);
    }
  }, {
    key: 'getWinnerInfo',
    value: function getWinnerInfo() {
      var winner = $(this.item).eq(17).find('img');

      return {
        name: winner.data('name'),
        luck: winner.data('luck'),
        image: winner.attr('src')
      };
    }
  }, {
    key: 'fill',
    value: function fill(users, winner, bank) {
      $(this.block).append(this.prepareItems(users, winner));
      $(this.parent).addClass(this.show.className());
      $(this.list).addClass(this.hide.className());
      $(this.winnerBank).html(bank + '$');
    }
  }, {
    key: 'finishHandler',
    value: function finishHandler() {
      $(this.parent).addClass(this.done.className());

      var winner = this.getWinnerInfo();

      $(this.winnerName).html(winner.name);
      $(this.winnerLuck).html(winner.luck + '%');
      $(this.winnerImage).attr('src', winner.image);
    }
  }, {
    key: 'clear',
    value: function clear() {
      $(this.parent).removeClass(this.done.className());
      $(this.parent).removeClass(this.show.className());
      $(this.list).removeClass(this.hide.className());
      $(this.block).empty();
    }
  }, {
    key: 'roll',
    value: function roll(callback) {
      var _this4 = this;

      $(this.block).css({
        marginLeft: this.getRollDistance()
      });

      setTimeout(function () {
        _this4.finishHandler();
...
var Banner = function () {
  function Banner(options) {
    ...
    var prefix = {
      nav: '_dots',
      link: '_dots_item',
      active: '_dots_item-active',
      list: '_list',
      item: '_list_item'
    };

    var defaultOptions = {
      nav: options.block + prefix.nav,
      link: options.block + prefix.link,
      active: options.block + prefix.active,
      list: options.block + prefix.list,
      item: options.block + prefix.item,
      delay: 5000
    };
  ...
  _createClass(Banner, [{
    ...
    value: function clickHandler(event) {
      this.removeIntervalEvent();
      $(this.item).hide();
      $(this.link).removeClass(this.active.className());
      $(event.target).addClass(this.active.className());
      $(this.item).eq($(event.target).index()).show();
      this.intervalEvent();
    }
  }, {
    key: 'intervalEvent',
    value: function intervalEvent() {
      var _this6 = this;
    ...
    key: 'removeIntervalEvent',
    value: function removeIntervalEvent() {
      clearInterval(this.interval);
    }
  }, {
    key: 'switchEvent',
    value: function switchEvent() {
      var isLast = $(this.active).index() == $(this.active).siblings().length;

      $(this.item).hide();

      if (isLast) {
        $(this.active).removeClass(this.active.className());
        $(this.link).eq(0).addClass(this.active.className());
        $(this.item).eq($(this.active).index()).show();
      } else {
        $(this.active).removeClass(this.active.className()).next().addClass(this.active.className());
        $(this.item).eq($(this.active).index()).show();
      }
    }
  }, {
    key: 'navEvent',
    value: function navEvent() {
      for (var i = 0; i < 3; i++) {
        $(this.nav).append('<a class="' + this.link.className() + '" href="#"></a>');
      }
    }
  }, {
    key: 'startEvent',
...
var Duel = function () {
  function Duel(options) {
    ...
    var prefix = {
      ...
      hide: '-hide',
      flip: '_content_flip',
      wreath: '_content_wreath',
      animated: '-animated',
      bar: '_bar'
    };

    var defaultOptions = {
      show: options.block + prefix.show,
      hide: options.block + prefix.hide,
      flip: options.block + prefix.flip,
      wreath: options.block + prefix.wreath,
      bar: options.block + prefix.bar,
      delay: 7050,
      animated: options.block + prefix.flip + prefix.animated,
      flash: options.block + prefix.wreath + prefix.animated,
      template: '<div class="modal-back modal-back-hide">\n          <div class="modal-duel">\n            <div class="modal-duel_content">\n              <div class="modal-duel_content_wreath"></div>\n              <div class="modal-duel_content_flip">\n                <img class="modal-duel_content_flip_image modal-duel_content_flip_image-first" src="{userFirstImage}">\n                <img class="modal-duel_content_flip_image modal-duel_content_flip_image-second" src="{userSecondImage}">\n              </div>\n              <div class="modal-duel_content_info">\n                <div class="modal-duel_content_info_title">В банке</div>\n                <div class="modal-duel_content_info_num">{infoSumm}$</div>\n              </div>\n            </div>\n            <div class="modal-duel_bar">{userFirstName} vs {userSecondName}</div>\n          </div>\n        </div>'
    };
  ...
  _createClass(Duel, [{
    ...
    value: function closeHandler() {
      ...
      $(this.block).removeClass(this.show.className()).addClass(this.hide.className());
      $(this.block).parent().addClass($(this.block).parent().attr('class').split(' ')[0] + '-hide');
      setTimeout(function () {
        $(_this7.block).parent().remove();
      }, 500);
    }
  }, {
    key: 'closeBind',
    value: function closeBind() {
      var _this8 = this;
    ...
    key: 'roll',
    value: function roll(info) {
      var _this9 = this;

      $('body').prepend(this.parseTemplate(info));

      this.showEvent();
      this.closeBind();

      $(this.flip).addClass(this.animated.className());
      $(this.wreath).addClass(this.flash.className());

      setTimeout(function () {
        _this9.stopEvent(info.user.second);
      }, this.delay);
    }
  }, {
    key: 'stopEvent',
    value: function stopEvent(user) {
      $(this.flip).removeClass(this.animated.className());
      $(this.wreath).removeClass(this.flash.className());
      $(this.bar).html('<span class="modal-duel_bar_won">Победил</span> ' + user.name);
    }
  }, {
    key: 'showEvent',
    value: function showEvent() {
      ...
      setTimeout(function () {
        $(_this10.block).addClass(_this10.show.className());
        $(_this10.block).parent().removeClass($(_this10.block).parent().attr('class').split(' ')[0] + '-hide');
      }, 0);
    }
  }, {
    key: 'parseTemplate',
    value: function parseTemplate(info) {
      var replacePattern = {
        userFirstImage: info.user.first.image,
        userSecondImage: info.user.second.image,
        userFirstName: info.user.first.name,
        userSecondName: info.user.second.name,
        infoSumm: info.summ
      };
      ...
      $.each(replacePattern, function (index, value) {
        modal = modal.replace('{' + index + '}', replacePattern[index]);
      });
...
var Won = function () {
  function Won(options) {
    ...
    var defaultOptions = {
      close: options.block + prefix.close,
      show: options.block + prefix.show,
      template: '<div class="modal-won">\n          <a href="#" class="modal-won_close"></a>\n          <div class="modal-won_content">\n            <div class="modal-won_content_wreath"></div>\n            <img class="modal-won_content_image" src="{userImage}">\n            <div class="modal-won_content_info">\n              <span class="modal-won_content_info_user">{userName}...</span>\n               выиграл {userSumm}$\n            </div>\n          </div>\n        </div>'
    };
  ...
  _createClass(Won, [{
    ...
    value: function closeBind() {
      ...
      $(this.close).click(function (event) {
        event.preventDefault();
        _this11.closeHandler();
      });
    }
  }, {
    key: 'prependEvent',
    value: function prependEvent() {
      $('body').prepend(this.parseTemplate());
    }
  }, {
    key: 'close',
    ...
    value: function showEvent() {
      ...
      setTimeout(function () {
        $(_this12.block).addClass(_this12.show.className());
      }, 0);
    }
  }, {
    key: 'parseTemplate',
  ...
  _createClass(Tabs, [{
    ...
    value: function clickBind() {
      ...
      $(this.item).click(function (event) {
        event.preventDefault();
        _this13.clickHandler(event);
      });
    }
  }, {
    key: 'startEvent',
    value: function startEvent() {
      $(this.item).eq(0).addClass(this.active.className());
      $(this.getTabsContent()[0]).show();
    }
  }, {
    key: 'getTabsContent',
    ...
    value: function hideEvent() {
      for (var i = 0; i < this.getTabsContent().length; i++) {
        this.getTabsContent()[i].hide();
      }
    }
  }, {
    key: 'clickHandler',
    value: function clickHandler(event) {
      this.hideEvent();
      $(this.item).removeClass(this.active.className());
      $(event.target).addClass(this.active.className());
      $($(event.target).attr('data-tab')).show();
    }
  }]);
...
;(function () {
  var $handler = $('.chat_form_input_emoji');
  var $target = $('.chat_form_emoji');
  var $list = $('.chat_form_emoji_list');
  var $next = $('.chat_form_emoji_controls_item-up');
  var $prev = $('.chat_form_emoji_controls_item-down');
  ...
  $list.owlCarousel({
    ...
    onInitialized: function onInitialized() {
      $next.addClass('chat_form_emoji_controls_item-disabled');
    }
  });

  $list.on('changed.owl.carousel', function (event) {
    $prev.removeClass('chat_form_emoji_controls_item-disabled');
    $next.removeClass('chat_form_emoji_controls_item-disabled');

    if (event.item.index === 0) {
      $next.addClass('chat_form_emoji_controls_item-disabled');
    }

    if (event.item.count === event.item.index + 4) {
      $prev.addClass('chat_form_emoji_controls_item-disabled');
    }
  });

  $handler.click(function (event) {
    event.preventDefault();
    $target.toggleClass('chat_form_emoji-hide');
  });
...
;(function (global) {
  ...
  var $list = $('.inventory_content_list');
  var $item = $('.inventory_content_list_item');
  ...
  function expandInventory() {
    ...
    var isExpandable = itemLength / 3 * itemHeight > height && containerHeight + itemHeight < windowHeight;
  ...
  function decreaseInventory() {
    ...
    var itemHeight = $item.outerHeight(true);
    var itemLength = $item.length;
    var isExpandable = height >= itemHeight * 4;
  ...
  global.expandInventory = expandInventory;
  global.decreaseInventory = decreaseInventory;
})(window);

;(function (global) {
  function openTicket(info) {
    var id = info.id;
    var user = info.user;
    var image = info.image;

    var $ticket = $('#ticket_' + id);
    var $user = $ticket.find('.lottery_list_item_info_winner_text_user');
    var $image = $ticket.find('.lottery_list_item_info_winner_image');
    var $status = $ticket.find('.lottery_list_item_main_link');

    $user.html(user);
    $status.html('Завершено');
    $image.attr('src', image);
    $ticket.addClass('lottery_list_item-opened');

    setTimeout(function () {
      $ticket.addClass('lottery_list_item-disabled');
    }, 1000);
  }
...
;(function (global) {
  ...
  function parseTemplate(_ref2) {
    var user = _ref2.user;
    var image = _ref2.image;
    var summ = _ref2.summ;
    var luck = _ref2.luck;

    return '\n      <div class="game_tape_list_item game_tape_list_item-hide">\n        <img src="' + image + '" class="game_tape_list_item_image">\n        <div class="game_tape_list_item_text">\n          <div class="game_tape_list_item_text_user">' + user + '</div>\n          <div class="game_tape_list_item_text_info">\n            <div class="game_tape_list_item_text_info_item game_tape_list_item_text_info_item-money">' + summ + '$</div>\n            <div class="game_tape_list_item_text_info_item game_tape_list_item_text_info_item-luck">' + luck + '%</div>\n          </div>\n        </div>\n      </div>\n    ';
  }

  function addWinner(config) {
    var $list = $('.game_tape_list');
    var $item = $('.game_tape_list_item');
    var offset = $item.eq(2).outerWidth(true);

    $list.animate({ paddingLeft: offset }, 250);
    $item.eq(2).addClass('game_tape_list_item-hide');

    setTimeout(function () {
      $list.prepend(parseTemplate(config)).css({ paddingLeft: 0 });

      setTimeout(function () {
        $list.find('.game_tape_list_item').eq(0).removeClass('game_tape_list_item-hide');
      }, 0);
    }, 300);
  }

  global.addWinner = addWinner;
})(window);
/*
var settings = {
  chat: {
...
$(window).keypress(function (event) {
  if (event.keyCode == 47) {
    var duel = new Duel(settings.duel);
    duel.roll(duelInfo);
  } else if (event.keyCode == 46) {
    gameProgressCircle.set(100);
    gameProgressBar.set(100);
    roulette.fill(users, winner, 1337);
    roulette.roll();
    setTimeout(function () {
      roulette.clear();
      gameProgressCircle.clear();
      gameProgressBar.clear();
    }, 7000);
  } else if (event.keyCode == 44) {
    var won = new Won(settings.won);
  }
});

$('.inventory_content_pag_controls_item-add').click(function (event) {
  event.preventDefault();
  expandInventory();
});

$('.inventory_content_pag_controls_item-remove').click(function (event) {
  event.preventDefault();
  decreaseInventory();
});
...
setTimeout(function () {
  openTicket({
    id: 1,
    user: 'test',
    image: '/img/lottery_list_item_main_image.png'
  });
}, 1000);*/